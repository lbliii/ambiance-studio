...
For multiple formats, use [iteration schema 2](../../templates/iteration-views.example.json) to render each view once and reuse its picture for shared soundtrack editions. Verify each movie against its bound revision/edition expectations. Present one complete delivery with explicit `?view=ID&role=ROLE` exact and current links, individual posters, and open checks for every pair. Partial deliveries require explicit scope notes. Do not copy a portrait creative verdict onto the landscape movie; shared source/PCM evidence can be cited where it actually applies. Existing project pipelines keep their copied criteria; adopting changed criteria requires an explicit recorded project revision.
