...
Run `plan coverage --stage animation` before calling canonical production scope ready for animation. It checks the current stage's expectations without requiring future exports. For an unmigrated legacy inventory, `plan check --require-complete` retains its inventory-only meaning. Keep unproduced required parts visible even when a partial draft is ready to show. Record observed readability and any unintended still intervals alongside the defect checks. Keep state checks, rendered-image checks, and encoded-video checks separate. Report the actual scene/asset hashes and engine version. The bundled example's Node check covers its own state only; a new scene needs its own inputs and suitable assertions.
