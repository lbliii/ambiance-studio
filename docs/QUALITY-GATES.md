...
For configured projects, legacy `scene` and `assets` folder watches (including explicit `scene/scene.json` and `assets/catalog.json` watches) also follow the current selected catalog, its image files and the complete packages of active model instances; scene watches include the selected scene. A generation switch therefore invalidates affected picture reviews even when the previous files remain intact. Missing or malformed selected inputs block a new passing picture review while independent gates remain readable. Unselected generations are not scanned. Exact restoration recomputes currentness without rewriting evidence. Captured revision reviews retain their own immutable dependency scope.
