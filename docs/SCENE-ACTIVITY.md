...
`sampled_windows` are half-open runs of changed projected state. Their onset, duration and rest are sampled candidates, not independent authored action schedules. A continuous route may stay active through several gestures; changing cel indices may select identical paint. Read exact timing holds, the action's direction, and the normal-speed proof together. For complete loops, quiet tail/head segments form one rest across the join; an entirely still loop counts once. Wrapped intervals carry `wraps_loop: true` and an end time beyond T on the next traversal. Active bouts and repeat gaps use the same circular rule. Partial segments remain linear. Do not treat an unknown first comparison in a partial segment or an interval between sparse samples as observed stillness.

## Raster attribution and warning calibration

For action layer set A, the renderer produces the target F and the scene with A disabled, Foff, using the same finishing pipeline. Attached descendants and source-driven followers respond through existing runtime semantics. Contribution is `F − Foff`. Temporal residual change compares that signed difference across two sampled times. Complete-loop residuals also seed frame zero with the last sampled target/disabled pair; partial-segment first residuals remain null. All metrics use the maximum absolute RGB channel delta per pixel in 8-bit sRGB; alpha is excluded. Full-frame change remains a separate column. A global pulse cannot automatically satisfy a character or environmental requirement.
...
The replay records fixed sixteen-layer, 320-pixel-stage single/paired workload costs, raster latency, peak process RSS, response bytes, saved frames, and declared playback sampling rate. Each run starts a fresh process; “warm” means the OS cache may be warm, not a controlled flushed-cache experiment. With activity proofs, `#panes` exposes `data-playback-metrics`: measured elapsed playback, distinct frame presentations, skipped frame steps, long tick gaps, configured fps and selected speed. It resets on seek/resume/speed change. Read the live browser result separately from the saved renderer benchmark; configured fps alone does not prove achieved playback. Timing numbers are not CI pass thresholds. This feature makes no renderer optimization or legacy pixel change; parity tests compare saved frames byte-for-byte with the existing view renderer.
