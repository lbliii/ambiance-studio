...
Use [layer and light planning](LAYER-AND-LIGHT-PLANNING.md) to establish complete characters, meaningful props, environment surfaces and depth families, then their internal action parts, foreground occluders and receiving surfaces. The seed is a reference; a full-scene derivative carrying unrelated objects is not a finished layer decomposition. Prepare production versions of every component needed for the chosen staging, including still subjects. Extract suitable paint; reconstruct hidden areas; generate new poses or expanded environments where needed. Choose generation extent from the production need and available authorization. Reuse a good cutout without forcing it to carry missing geometry.
