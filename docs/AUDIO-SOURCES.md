...
This receipt deliberately pins the first project's selected origin as well as its preserved copy. The implemented [LIB-01 audio library](AUDIO-LIBRARY.md) copies pinned source bytes into a destination and creates a distinct typed materialization receipt. It preserves this prior receipt verbatim as archival provenance and validates active destination-contained dependencies without requiring the former project or library. Source-preparation v1 is unchanged.
