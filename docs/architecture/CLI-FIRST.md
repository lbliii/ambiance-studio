...
The [consolidated production improvements](PRODUCTION-IMPROVEMENTS.md) and [dependency backlog](PRODUCTION-IMPROVEMENTS.yaml) now organize the next work across semantic planning, explicit expectations, dual-format staging, asset preparation, source/follower lighting, activity evidence, CI and agent behavior trials. They distinguish existing local changes from planned capabilities and reuse the detailed component plans below.
