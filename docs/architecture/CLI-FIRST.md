...
September 13 clarification: the broader goal is [agent-operated traditional animation](AGENT-ANIMATION-VISION.md). Use familiar model-sheet, pose, exposure-sheet, pencil-test and retake concepts over the native production state. Planned P15 adds animation authoring/inspection; P16 connects actual visual/listening observations to revisions. These extend the existing command, evaluator and feedback owners. Their acceptance includes a full-minute film and ordinary directing/reuse tasks; command execution alone does not establish animator competence.
