...
| --- | --- | --- | --- |
| High | [revisions.py](../../ambiance_studio/revisions.py) is 644 lines and owns schema helpers, path/reference utilities, dependency collection, captures, editions, review contexts and command routing. Many unrelated modules import `fields`, `identifier`, `seal`, `relative` and other helpers from this domain module. | Extract contract sealing/validation and project reference identity first, then separate revision capture from edition/review operations. Preserve existing serialized bytes and compatibility exports. | Existing revision/edition/tamper tests plus compatibility fixtures for persisted seals, schema versions and path containment. |
