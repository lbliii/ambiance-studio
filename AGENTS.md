...
- For full layered films, extract complete characters and meaningful props/environment surfaces before choosing animated subparts. A still body belongs to its character rig; a head or neck patch is not full-character extraction. Follow `docs/LAYER-AND-LIGHT-PLANNING.md` for production-only and whole-object isolation/removal proofs, then use intended actions and overlaps to choose internal articulation. Explicit component-only scope may remain partial.
