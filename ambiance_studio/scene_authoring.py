...
def resolve_batch(project,scene,catalog,batch):
    ...
    def check_asset(aid):
        ...
        if recipe_ref:
            ...
            for key in ['sources','registration_source','source_mapping','edge_preparation','motion_preparation','preparation_receipt','cel_trim', 'region_receipt']:
            ...
            for label in ['registration_source','source_mapping','edge_preparation','motion_preparation','preparation_receipt','cel_trim', 'region_receipt']:
                ...
                if ref:
                    ...
                    if label=='region_receipt':
                        from .art_regions import validate_region_receipt
                        region=validate_region_receipt(_path(project,relative),ref['sha256'],[_path(project,recipe_relative(recipe_file,name)) for name in source_names],recipe)
                        for dep in region['dependencies']: pin(str(dep['path'].relative_to(project)),dep['sha256'],dep['role'])
                    if label=='preparation_receipt':
                        from .compound_preparation import validate_preparation_receipt
                        checked=validate_preparation_receipt(_path(project,relative),ref['sha256'],[_path(project,recipe_relative(recipe_file,name)) for name in source_names],recipe)
                        for dep in checked['dependencies']: pin(str(dep['path'].relative_to(project)),dep['sha256'],dep['role'])
                    ...
                    if label=='cel_trim':
                        ...
                        trimmed=validate(_path(project,relative),ref['sha256'],[_path(project,recipe_relative(recipe_file,name)) for name in source_names],recipe)
                        for dep in trimmed['dependencies']: pin(str(Path(dep['path']).relative_to(project)),dep['sha256'],dep['role'])
    ...
    for aid in set(layers.values()):
        if any(assets.get(aid,{}).get('provenance',{}).get(k) for k in ['motion_preparation','preparation_receipt','cel_trim', 'region_receipt']): check_asset(aid)
    if has_finishing:
