...
def resolve_batch(project,scene,catalog,batch):
    ...
    def check_asset(aid):
        ...
        if recipe_ref:
            ...
            for key in ['sources','registration_source','source_mapping','edge_preparation','motion_preparation','preparation_receipt','cel_trim']:
            ...
            for label in ['registration_source','source_mapping','edge_preparation','motion_preparation','preparation_receipt','cel_trim']:
    ...
    for aid in set(layers.values()):
        if any(assets.get(aid,{}).get('provenance',{}).get(k) for k in ['motion_preparation','preparation_receipt','cel_trim']): check_asset(aid)
    if has_finishing:
