...
def dependency_roles(scene):
    ...
    for scope in ['lights', 'shadows', 'reflections']:
...
def _rig_fragment(scene, finish):
    """Capture declared roots and attachment ancestors, never nearby objects."""
    required = relationship_ids(finish); by_layer = {row['id']: row for row in scene['layers']}
...
def _requirements(finish, rig=None):
    required = relationship_ids(finish); roles = dependency_roles({'finishing': finish})
    if rig is not None:
...
def _validate_rig(rig, finish):
    ...
    selected = _rig_fragment(rig, finish)
...
def export_package(project, scene, catalog, out, include_rig=False):
    ...
    rig = _rig_fragment(scene, finish) if include_rig else None
    requirements, roles = _requirements(finish, rig); by_id = {a['id']: a for a in catalog['assets']}
    ...
    doc = _seal({'kind': PACKAGE, 'version': 1, 'finishing': finish, 'requires': requirements, 'source': source,
                 **({'rig': rig} if rig is not None else {})})
...
def import_batch(project, scene, catalog, package, bindings, include_rig=False):
    ...
    doc = studio.read(package_path); fields(doc, ['kind', 'version', 'finishing', 'requires', 'source', 'payload_sha256', 'rig'], 'look package')
    ...
    if 'rig' in doc:
        ...
        _validate_rig(rig, doc['finishing'])
    ...
    expected, roles = _requirements(doc['finishing'], rig)
    ...
    for record in asset_records:
        fields(record, ['id', 'sha256', 'width', 'height', 'atlas', 'roles', 'rig_metadata'], 'package asset identity')
    ...
    for record in asset_records:
        ...
        if mask and not identical: raise ValueError(f'Mask version differs for binding {record["id"]} -> {target["id"]}')
        if 'rig-image' in record['roles'] and (not identical or record['rig_metadata'] != {'pivot': target.get('pivot'), 'sockets': target.get('sockets', {})}): raise ValueError(f'Rig image version/cell layout/socket metadata differs for binding {record["id"]} -> {target["id"]}')
    ...
    candidate = scene_bridge('apply', scene, catalog, {'batch': batch}) if rig is not None else {**scene, 'finishing': result}
