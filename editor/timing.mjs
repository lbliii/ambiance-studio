...
export function sceneTiming(scene,catalog,{layer:filter}={}){
  ...
  for(let f=0;f<N;f++)for(const state of rig.sample(f/fps))if(ids.has(state.id)){
  ...
  const layers=selected.map(layer=>{
    ...
    return {layer:layer.id,asset:layer.asset,timing_driver:driver,fallback_cycle:fallback,
      ...
      join:{declared_policy:layer.track_loop||'closed',leading_nonrendered_frames:leading,trailing_nonrendered_frames:trailing,
        first_render_eligible:first.render_eligible,last_render_eligible:last.render_eligible}};
  });
  return {version:1,engine_version:ENGINE_VERSION,scene:scene.id,picture_seconds:T,output_fps:fps,output_frames:N,layers,
