...
def build(recipe_path, out):
    ...
    if any(json.loads(recipe_path.read_text()).get(k) for k in ['motion_preparation', 'preparation_receipt', 'region_receipt']) and not out.exists():
...
def admit(pack, catalog_path):
    ...
    if any(asset.get('provenance',{}).get(k) for k in ['motion_preparation', 'preparation_receipt', 'region_receipt']):
        sys.path.insert(0,str(Path(__file__).resolve().parents[1]))
