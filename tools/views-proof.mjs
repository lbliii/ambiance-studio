export function viewsProofPage(views, fps, start, frames, loopSeconds) {
  const payload = JSON.stringify({views, fps, start, frames, loopSeconds}).replaceAll('<', '\\u003c');
...
Promise.all(panes.flatMap(p=>p.images.map(image=>image.decode()))).then(()=>{draw(0);play.disabled=false;last=performance.now();function tick(now){if(playing){clock=(clock+Math.max(0,now-last)/1000*Number(document.querySelector('#speed').value))%(config.frames/config.fps);draw(Math.floor(clock*config.fps));}last=now;requestAnimationFrame(tick);}requestAnimationFrame(tick);}).catch(error=>{status.textContent='Could not load proof: '+error.message;});
play.onclick=()=>{playing=!playing;last=performance.now();play.textContent=playing?'Pause':'Play';};seek.oninput=()=>{playing=false;play.textContent='Play';const frame=Number(seek.value);clock=frame/config.fps;draw(frame);};
</script></html>`;
}
