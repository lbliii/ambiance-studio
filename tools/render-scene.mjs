...
function htmlProof(files, fps, width, height, disabled, labels = null) {
  const payload =
      JSON.stringify({files, fps, width, height, disabled, labels}).replaceAll('<', '\\u003c');
...
<style>body{background:#14151b;color:#eee;font:16px system-ui;margin:24px}main{display:flex;gap:16px;flex-wrap:wrap}figure{margin:0}canvas{display:block;max-width:100%;height:auto;background:#000;width:${
      width}px}figcaption{margin:8px 0}button,select,input{font:inherit;margin:6px}p{max-width:70ch}</style>
...
async function main() {
  ...
  const report = {
    ...
    renderer_sha256 : sha(await fs.readFile(fileURLToPath(import.meta.url))),
  ...
  if (mode === 'look-proof') {
    ...
    const child = own(spawn(nativePath,
                            [
    ...
    try {
      for (let frame = 0; frame < encodedFrames; frame++) {
        ...
        if (stdinError)
          throw stdinError;
        if (!child.stdin.write(canvas.data()))
          await Promise.race([
    ...
    report.native_encoder = JSON.parse(stdout);
