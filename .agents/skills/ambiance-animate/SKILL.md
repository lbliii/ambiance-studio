...
Before tuning motion, use `render rig-proof` for [whole-object isolation/removal](../../../docs/LAYER-AND-LIGHT-PLANNING.md#whole-object-ownership-and-proof), then internal rest/extreme and fixed-occluder comparisons. Hiding a character must remove its still body as well as its animated head; inspect every contributing layer for residual paint. Do not parent production rigs to a removable reference/fallback layer. Keep fixed glass and receiving shadows independent. Judge repeated flames' shape, cadence and phase together; compare supporting motion enabled/disabled when its readability is uncertain.
...
Close `animation` from technical evidence, `plan coverage --stage animation` for canonical declared scope, and actual visual review in the requested compositions. For an unmigrated legacy inventory, `plan check --require-complete` retains its narrower inventory-only meaning. Name observed story actions and too-still intervals as well as defects. Changes to assets or camera limits require a new affected review. Pure JSON validation cannot establish that a frame looks correct or that motion is noticeable. See [seed-to-stage production](../../../docs/SEED-TO-STAGE.md).
